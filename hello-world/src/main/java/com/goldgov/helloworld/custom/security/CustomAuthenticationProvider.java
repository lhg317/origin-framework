...
public class CustomAuthenticationProvider extends BaseAuthenticationProvider{
	...
	protected boolean authenticate(String loginName, String password) throws Exception {
		String originPwd = authAccountService.getPassword(loginName);
		String encodePwd = passwordEncoder.encode(password);
		if(originPwd.equals(encodePwd)){
			return true;
		}
		return false;
	}
	...
	protected UserToken getUserToken(String loginName, String password) {
		...
		UserToken userToken = new UserToken(loginName,password,user.getUserName());
		return userToken;
	}
